...
插件已升级支持直接从 AstrBot 的日志流（LogBroker）获取日志，实现**零延迟的实时日志监控**！
...
**检查状态：**
```bash
curl "http://127.0.0.1:7199/api/health?token=your_token" | jq
```
...
- 默认极简总览：系统健康、关键运行态、少量重要事件和实时工具
- 详细状态折叠展示资源概览和运行状态
- 大屏关键态：活动会话、运行工具、最近发送、慢请求和错误事件
- AstrBot 页用二级标签页展示会话、模型和日志维度可视化
- 健康诊断：资源阈值、日志可读性、最近错误和平台可用性
- **实时日志流**：零延迟日志监控（LogBroker 模式）或文件读取（兼容模式）
- 基于 trace 日志识别收到消息、规则选择、工具调用、工具返回、发送回复、慢请求和错误事件
- 基于普通日志识别平台收到消息、警告和错误，补充 trace 没覆盖的运行信息
- 重要信息聚焦工具调用、工具返回、发送回复、慢请求、警告和错误，支持事件类型筛选、右侧证据详情、原文定位和同一 `span_id` 会话链路
