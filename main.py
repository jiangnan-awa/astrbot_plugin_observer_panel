...
def _safe_attr(obj: Any, name: str, default: Any = None) -> Any:
    """
    安全获取对象属性，失败时返回默认值。
    区分 AttributeError（属性不存在）和其他异常（访问错误）。
    """
    try:
...
class ObserverPanelPlugin(Star):
    ...
    async def _handle_index(self, request: web.Request) -> web.Response:
        return await self._send_file(self.web_dir / "index.html", content_type="text/html")
    ...
    async def _handle_static(self, request: web.Request) -> web.Response:
        ...
        mtime_str = time.strftime("%a, %d %b %Y %H:%M:%S GMT", time.gmtime(stat.st_mtime))
        etag = f'W/"{stat.st_size}-{int(stat.st_mtime)}"'
        self._static_cache[cache_key] = (data, mtime_str, etag)
        response = web.Response(text=data, content_type=content_type)
        response.headers["Cache-Control"] = "public, max-age=3600"
        response.headers["ETag"] = etag
        response.headers["Last-Modified"] = mtime_str
    ...
    async def _handle_logs(self, request: web.Request) -> web.Response:
        ...
        # 如果启用了 LogBroker，优先使用日志流
        if self._use_log_stream and source in {"all", "live", "stream"}:
    ...
    def _collect_network_info(self, *, compact: bool = False) -> dict[str, Any]:
        ...
        for path in sorted(Path("/sys/class/net").glob("*")):
            ...
            item = {
                ...
                "mtu": _as_int(_read_text_sync(path / "mtu"), 0),
                "mac": _read_text_sync(path / "address"),
    ...
    async def _get_stream_logs(self, limit: int = 300, time_window_minutes: int = 10) -> list[dict[str, Any]]:
        """从日志流缓存获取最近的日志
    ...
    async def _handle_logs_live(self, request: web.Request) -> web.Response:
        ...
        # 获取缓存的日志
        logs = await self._get_stream_logs(500)
